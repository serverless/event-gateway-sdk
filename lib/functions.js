      ...
      if (response.status !== 201) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(
                `Failed to create the function ${params.functionId} due the error: ${errorMessage}`
              )
            } else {
              throw new Error(
                `Failed to create the function ${params.functionId} and couldn't parse error body.`
              )
            }
          })
      }
      ...
      if (response.status !== 200) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(
                `Failed to update the function ${params.functionId} due the error: ${errorMessage}`
              )
            } else {
              throw new Error(
                `Failed to update the function ${params.functionId} and couldn't parse error body.`
              )
            }
          })
      }
...
module.exports.list = (config, filters) => {
  const path = `/v1/spaces/${config.space}/functions?${querystring.stringify(filters)}`
