    ...
    .then(response => {
      if (response.status !== 200) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(`Failed to update ${params.corsId} CORS configuration due the error: ${errorMessage}`)
            } else {
              throw new Error(`Failed to update ${params.corsId} CORS configuration and couldn't parse error body.`)
            }
          })
      }
      return response.json()
    ...
    .then(response => {
      if (response.status !== 204) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(`Failed to delete ${params.corsId} CORS configuration due the error: ${errorMessage}`)
            } else {
              throw new Error(`Failed to delete ${params.corsId} CORS configuration and couldn't parse error body.`)
            }
          })
      }
      return undefined
    ...
    .then(response => {
      if (response.status !== 200) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(`Failed to fetch the CORS configurations list due the error: ${errorMessage}`)
            } else {
              throw new Error("Failed to fetch the CORS configurations list and couldn't parse error body.")
            }
          })
      }
