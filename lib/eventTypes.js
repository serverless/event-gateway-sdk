      ...
      if (response.status !== 201) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(
                `Failed to create ${params.name} event type due the error: ${errorMessage}`
              )
            } else {
              throw new Error(
                `Failed to create ${params.name} event type and couldn't parse error body.`
              )
            }
          })
      }
      ...
      if (response.status !== 200) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(
                `Failed to update ${params.name} event type due the error: ${errorMessage}`
              )
            } else {
              throw new Error(
                `Failed to update ${params.name} event type and couldn't parse error body.`
              )
            }
          })
      }
      ...
      if (response.status !== 204) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(
                `Failed to delete ${params.name} event type due the error: ${errorMessage}`
              )
            } else {
              throw new Error(
                `Failed to delete ${params.name} event type and couldn't parse error body.`
              )
            }
          })
      }
