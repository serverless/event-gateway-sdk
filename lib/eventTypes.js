    ...
    .then(response => {
      if (response.status !== 201) {
        ...
        return response
          ...
          .catch(() => {
            if (errorMessage) {
              throw new Error(`Failed to create ${name} event type due the error: ${errorMessage}`)
            } else {
              throw new Error(`Failed to create ${name} event type and couldn't parse error body.`)
            }
          })
      }
